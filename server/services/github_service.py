...
class GithubService:
    ...
    def get_new_folder_path(self, document: dict) -> str:
        ...
        if ("organisation" in document.keys()
            ...
            folder_path = (
                f"{self.file_folder}/{platform_name}"
                f"/{organisation_name}"
            )
            ...
            organisation_name = document["organisation"]["name"]
            folder_path = (
                f"{self.file_folder}/{self.platform_name}/"
                f"{organisation_name}"
            )
    ...
    def commit_file(self, request_data: dict) -> str:
        ...
        file_path = f"{self.platform_name}/{self.organisation_name}"
    ...
    def get_folders_files(self) -> dict:
        """
        ...
        for i, tree in enumerate(trees):
            filename = tree["path"]
            project_id = re.search(r"\d+", filename)
    ...
    def delete_folder_files(self, folder_tree: dict, new_folder_path: str):
        """
        Delete files of tree with outaded organisation/platform names
        ...
        """
        file_path = f"{self.platform_name}/{self.organisation_name}"
        folder_path, platform_name, organisation_name = (
    ...
    def get_folder_tree_request(self) -> str:
        ...
        organisation/platform name is being updated

        Returns:
        tree_request_url -- GitHub API request string of
                            tree which organisation/platform
                            name is being updated
        """
